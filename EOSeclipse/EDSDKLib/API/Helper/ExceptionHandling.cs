...
namespace EOSDigital.API
{
    ...
    public static class ErrorHandler
    {
        ...
        /// <summary>
        /// Checks for an error in SDK calls and checks how to treat it, except DEVICE_BUSY is ignored
        /// </summary>
        public static void CheckErrorCont(object sender, ErrorCode errorCode)
        {
            if (errorCode == ErrorCode.OK) return;
            else if (errorCode == ErrorCode.DEVICE_BUSY)
            {
                //Console.WriteLine("#{0}# {1:HH:mm:ss.fff}", errorCode, DateTime.Now); 
                return;
            }
            else
            {
