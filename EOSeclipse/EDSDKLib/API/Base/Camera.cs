...
namespace EOSDigital.API
{
    ...
    public class Camera : IDisposable
    {
        ...
        public void SendCommandCont(CameraCommand command, int inParam = 0, int delay = 100, int errors = 10)
        {
            ...
            MainThread.Invoke(() =>
            {
                //Console.WriteLine("{0:HH:mm:ss.fff}  - 0", DateTime.Now);
                ...
                while (errorCount <= errors && errorCode == ErrorCode.DEVICE_BUSY)
                {
                    ...
                    errorCode = CanonSDK.EdsSendCommand(CamRef, command, inParam);
        ...
        /// <exception cref="SDKStateException">Canon SDK is not initialized</exception>
        /// <exception cref="SDKException">An SDK call failed</exception>
        public string GetStringSetting(PropertyID propID, int inParam = 0, List<String> handleCodes = null)
        {
            CheckState();
            try
            {
            ...
            {
                if (handleCodes != null && handleCodes.Contains(ex.Message.ToString()))
                {
